...
def compute_a(z):
    ...
    z1[z1 < -700] = -500
    if np.sum(z1 > 700) > 0:
        a = np.asmatrix(np.zeros(z.shape[0])).T
        c = np.asmatrix(z1[np.where(z1 > 700)] / np.sum(z1[np.where(z1 > 700)])).T
        if np.sum(c > 0.65) > 0:
            a[np.where(c > 0.65)] = 1.0
        else:
            for i in np.where(z1 > 700):
                a[i] = 1 / len(np.where(z1 > 700))

    else:
        a = np.asmatrix(np.exp(z1) / (np.sum(np.exp(z1))))
...
def compute_da_dz(a):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    list_1 = [float(a[i]*(1-a[j])) if i == j else float(-a[i]*a[j]) for i in range(len(a)) for j in range(len(a))]
    da_dz = np.asmatrix(list_1).reshape((a.shape[0],a.shape[0]))
