...
def compute_a1(z1):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    list_1 = [0 if i <= -10 else 1 if i >= 500 else float(1/(1+np.exp(-i))) for i in z1]
    a1 = np.asmatrix(list_1).T
...
def compute_da1_dz1(a1):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    list_1 = [float(i*(1-i)) for i in a1]
    da1_dz1 = np.mat(list_1).reshape((a1.shape[0],1))
